...
class SRC(Trainer):
    ...
    Arguments:
        n_1 (int): a mini-batch size for the estimation of stochastic gradient
        n_2 (int): a mini-batch size for the estimation of stochastic Hessian-vector products
    """

    def __init__(self, n_1, n_2, *args, **kwargs):
    ...
    def step(self):
        # Sample S_1, S_2
        # g
        # Write the function for the Hessian - vec product
        # Use cubic solver
        # update
        # If enough progress is made - final sub-solver
        # Use cubic solver, finalizing
        return

    estimate_stepsize = Ad_grad.estimate_stepsize

    def init_run(self, *args, **kwargs):
        super(SRC, self).init_run(*args, **kwargs)
